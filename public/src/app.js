...
const PHASES = [
  ...
  },
  {
    ...
    block: 3,
    focus:
      "Before you call it ready, run the full check. This is where PulpitOS makes sure nothing is missing - the gospel, Christ as hero, illustrations, specific application, the invitation, and a clear landing.",
    doItems: [
...
function renderTopbar(active) {
  const status = ui.serverStatus.checked
    ? ui.serverStatus.configured
      ? "ready"
      : "missing"
    : "";
  const statusLabel = ui.serverStatus.checked
    ? ui.serverStatus.configured
      ? "Coach connected"
  ...
  return `
    <header class="topbar">
      <div class="brand">
        <img class="brand-mark" src="./assets/icon-gray.png" alt="" />
        <div class="brand-text">
          <div class="brand-name">Pulpit<span>OS</span></div>
          <div class="brand-kicker">From the text to the pulpit.</div>
        </div>
      </div>
      <div class="top-actions">
        <span class="status-line" title="${attr(statusLabel)}"><i class="status-dot ${status}"></i>${escapeHtml(statusLabel)}</span>
        ${
  ...
  return `
    <section class="empty-state">
      <div class="panel empty-hero">
        <img src="./assets/icon-gray.png" alt="" />
        <div>
          <span class="eyebrow">Sermon workspace</span>
          <h1 class="title">PulpitOS</h1>
        </div>
        <p>A guided prep desk for exegesis, research, outline, application, manuscript review, and delivery.</p>
    ...
    updateActive({
      thread: [
        ...next,
        {
          role: "assistant",
          content:
            "I could not reach the coach server. Start the local server and set ANTHROPIC_API_KEY to use coaching.",
        },
      ],
    });
  } finally {
...
async function reviewSermon() {
  ...
  try {
    ...
    ui.reviewResult =
      "I could not reach the review server. Start the local server and set ANTHROPIC_API_KEY to use review.";
  } finally {
...
function deleteActive() {
  const active = getActive();
  if (!active) return;
  const ok = confirm(`Remove "${active.passage || "this sermon"}" from PulpitOS?`);
  if (!ok) return;
...
document.addEventListener("click", (event) => {
  ...
  if (action === "export-all") {
    ...
    downloadText("pulpitos-sermons.md", all || "# PulpitOS\n");
  }
