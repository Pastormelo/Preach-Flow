...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    res.status(500).json({ text: error.message || "Review request failed." });
  }
};
...
async function callAnthropic({ system, messages, maxTokens }) {
  ...
  if (!key) {
    return {
      configured: false,
      text:
        "The PulpitOS server is running, but ANTHROPIC_API_KEY is not set. Add it as an environment variable to enable coaching and review.",
    };
  }
  ...
  if (!response.ok) {
    return {
      configured: true,
      text: `Anthropic returned ${response.status}: ${data.error?.message || "request failed"}`,
    };
  }
  ...
  return {
    ...
    text:
      data.content
        ...
        .join("\n")
        .trim() || "No text response came back.",
  };
}
