...
server.listen(PORT, "127.0.0.1", () => {
  ...
  if (!process.env.OPENAI_API_KEY) {
    console.log("OPENAI_API_KEY is not set, so coach/review calls will return setup guidance.");
  }
});
...
async function callOpenAI({ system, messages, maxTokens }) {
  ...
  if (!key) {
    return {
      configured: false,
      text:
        "The local Preach Flow server is running, but OPENAI_API_KEY is not set. Restart it with OPENAI_API_KEY in the environment to enable coaching and review.",
    };
  }
